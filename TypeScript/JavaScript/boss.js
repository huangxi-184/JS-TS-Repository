...
const startPost = async () => {
    try {
        let bigJobList = []
        for (let i = 1; i <= 10; i++) {
            let data = await axios.get(`https://www.zhipin.com/wapi/zpgeek/search/joblist.json?
        ...
        fs.writeFile('./boss1.json', JSON.stringify({
            "allData": bigJobList
        }), function (err, data) {
            if (err) {
                throw err
